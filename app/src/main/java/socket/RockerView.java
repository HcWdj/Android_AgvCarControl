...
public class RockerView extends View {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:// 移动
                ...
                if(null!=mOnLocationListener){
                    ...
                    if(getY>2*yOffset)
                        getY=2*yOffset;
                    finalX=(int)((getX-xOffset)*127/xOffset);
                    finalY=(int)((getY-yOffset)*127/yOffset);

                    //finalX=(getX-xOffset)>0?(int)((getX-xOffset)*127/xOffset):(int)((getX-xOffset)*128/xOffset);
                    //finalY=(getY-yOffset)>0?(int)((getY-yOffset)*127/yOffset):(int)((getY-yOffset)*128/yOffset);
                    if(finalX<=20 && finalX>=-20)
                        finalX=0;
                    if(finalY<=20 && finalY>=-20)
                        finalY=0;

                    mOnLocationListener.onLocation(finalX,finalY);
                }
                break;
            case MotionEvent.ACTION_UP:// 抬起
                moveX=getMeasuredWidth()/2;
                moveY=getMeasuredHeight()/2;
                if(null!=mOnLocationListener){
                    float getX=moveX;
                    float getY=moveY;
                    mOnLocationListener.onLocation(getX-xOffset,getY-yOffset);
                }
     ...
     * ------------
     */
    public void setOnLocation(OnLocationListener listener){
